...
- [x] 父任务 7 条已逐条标注证据位置。CI 已双绿(run 30248517365)。
      仍有 1 条带限定:Claude Code 会话可见性未实机验证(仅 Codex 侧实测),已写明。
- [x] 标签 `dev-project-local-v0.1.0`:2026-07-27 用户决定**暂不创建**,等 CI 结果。
      CI 已双绿,标签仍未创建,留待用户后续拍板。结论记录在此。
